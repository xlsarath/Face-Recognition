...
def checkUnknown(result):
    global count
    if(result=='Unknown' and count < 1):
        
        gmail_user = 'XX@gmail.com'
        gmail_password = 'XX'

        sent_from = gmail_user
        to = ['xlsarathchandra@gmail.com']
        subject = 'Alert from home monitoring app!!'
        body = 'Hi There, \n unkown person detected in the lobby @'+datetime.now().strftime("%Y-%m-%d %H:%M:%S")+'\n login into app to find realtime feed'
        email_text = """\
        From: %s
        To: %s
        Subject: %s
        ...
        log_file.write("Unknown person detected @"+datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        log_file.close()
        count += 1
        account_sid = ''#enter ur account sid from twilio
        auth_token = '' #auth_token here
        client = Client(account_sid, auth_token) 
        message = client.messages.create( 
							from_='use num given by twilio', 
							body ='Unkown Face Detected!!', 
							to ='6696696696969'
						) 
        ...
        p = Popen("C:\\Users\\psma018\\.anaconda\\navigator\\scripts\\project295B\\powershell_shortcut.bat",shell=True, stdout = subprocess.PIPE)

        stdout, stderr = p.communicate()
        print(p.returncode)
        #os.system("C:\\Users\psma018\\Desktop\\FaceRec-master\\faceRecog.bat")    
